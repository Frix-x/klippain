...
-> For ERCF users with ["Happy Hare"](https://github.com/moggieuk/ERCF-Software-V3) config installed, you must specify `INITIAL_TOOL` in your slicer start G-code. Base on the previous example for SuperSlicer:
```
...
```
And optionally you can specify if you want or not unloading filament at the end of the print by using `ERCF_UNLOAD_AT_END=1` or `ERCF_UNLOAD_AT_END=0`. For example for SuperSlicer:
```
END_PRINT ERCF_UNLOAD_AT_END=0
```
Rem: By default, Klippain unloads the filament at the end of the print; you can choose the default behavior by modifying the variables values in `variables.cfg`.
